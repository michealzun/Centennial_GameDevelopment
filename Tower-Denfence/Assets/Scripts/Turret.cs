...
public class Turret : MonoBehaviour {
	...
	void UpdateTarget()
	{
		...
		foreach(GameObject enemy in enemies)
		{
			...
			if(distanceToEnemy < shortestDistance)
			{
				...
				neareastEnemy = enemy;
                animator.SetBool("targetLock", true);
			}
		}
		...
		{
			target = null;
            animator.SetBool("targetLock", false);
        }
	}
    ...
    public void FindEnemyState_Start()
    {
        CancelInvoke();
        InvokeRepeating("UpdateTarget", 0f, 0.5f);
    }

    public void FireState_Update()
    {
        if (animator.GetBool("targetLock") == false)
        {
            if (useLaser)
            {
                if (lineRenderer.enabled)
